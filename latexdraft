Partial dump of LateX draft. I am re-writing and re-structuring some things, so I am saving a partial current version here in case I decide to use some of the text I am planning on deleting.
...
The project undertaken in this paper is to adapt the Morpheus programming model to work with a high level programming language in a manner that is transparent for application developers. We target the Python programming language, primarily for its simplicity, ease of use, and high popularity among application developers.
...
Research into possible host applications for these in-storage computational resources is of special relevance to our work, which seeks to make the benefits of in-storage computing more accessible.

\subsection{Big Data Analytics}

One very popular area of application for in-storage computing is big data analytics. Big data applications are heavily bottlenecked by storage bandwidth. Because in-storage processors can access this data before it reaches the main bus however, using in-storage processing to perform initial filtering of the data has the potential to greatly reduce traffic between the host and the storage device. Choi and Kee demonstrate the potential benefits of this storage-level data filtering using model analysis in "Energy Efficient Scale-In Clusters with In-Storage Processing for Big-Data Analytics". Aside from performance improvements, they also demonstrate the potential for reducing energy consumption, which is dominated by the transfer of data.

\subsection{User-programmable SSDs}

Other projects have sought to give developers more general access to in-storage processors, as opposed to designing application-specific programs.

Seshadri et al. achieve this goal by designing a system called Willow. Willow consists of a system driver, a SSD with a custom interface, and a template for user programs called SSD-Apps, which handles many of the complexities of writing in-storage code for the user.
